...
namespace {
    YARP_LOG_COMPONENT(GAZEBOCONTROLBOARD, "gazebo-yarp-plugins.plugins.ControlBoard")
}
...
GZ_REGISTER_MODEL_PLUGIN(GazeboYarpControlBoard)
    ...
    void GazeboYarpControlBoard::Load(physics::ModelPtr _parent, sdf::ElementPtr _sdf)
    {
        ...
        if (!_parent) {
            yCError(GAZEBOCONTROLBOARD) << "GazeboYarpControlBoard plugin requires a parent.";
            return;
        }
        ...
        for (int n = 0; n < netList->size(); n++)
        {
            ...
            if( newPoly.poly != NULL)
            {
                ...
                yCWarning(GAZEBOCONTROLBOARD) << "controlBoard"  << newPoly.key.c_str() << "already opened.";
            }
            else
            {
